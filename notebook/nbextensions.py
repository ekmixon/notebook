...
def _set_nbextension_state(section, require, state,
    ...
    if logger:
        logger.info(
            f'{"Enabling" if state else "Disabling"} {section} extension {require}...'
        )
...
def validate_nbextension_python(spec, full_dest, logger=None):
    ...
    if require is not None:
        require_path = os.path.join(full_dest[:-len(spec["dest"])], f"{require}.js")
    ...
    def install_extensions(self):
        ...
        if full_dests := install(
            ...
            **kwargs,
            ...
            self.log.info(
                f'\nTo initialize this nbextension in the browser every time the notebook (or other app) loads:\n\n      jupyter nbextension enable {self.extra_args[0] if self.python else "<the entry point>"}{" --user" if self.user else ""}{" --py" if self.python else ""}{" --sys-prefix" if self.sys_prefix else ""}\n'
            )
...
def _get_nbextension_dir(user=False, sys_prefix=False, prefix=None, nbextensions_dir=None):
    ...
    if len(conflicting_set) > 1:
        raise ArgumentConflict(
            f"cannot specify more than one of user, sys_prefix, prefix, or nbextensions_dir, but got: {', '.join(conflicting_set)}"
        )

    if user:
        return pjoin(jupyter_data_dir(), u'nbextensions')
    elif sys_prefix:
        return pjoin(ENV_JUPYTER_PATH[0], u'nbextensions')
    elif prefix:
        return pjoin(prefix, 'share', 'jupyter', 'nbextensions')
    elif nbextensions_dir:
        return nbextensions_dir
    else:
        return pjoin(SYSTEM_JUPYTER_PATH[0], 'nbextensions')
...
def _get_nbextension_metadata(module):
    ...
    if not hasattr(m, '_jupyter_nbextension_paths'):
        raise KeyError(
            f'The Python module {module} is not a valid nbextension, it is missing the `_jupyter_nbextension_paths()` method.'
        )
